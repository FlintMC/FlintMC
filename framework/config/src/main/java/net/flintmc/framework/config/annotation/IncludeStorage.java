...
/**
 * Acts as a whitelist for {@link ConfigStorage}s inside a {@link Config}, more information can be
...
public @interface IncludeStorage {
  ...
  /**
   * An array of storages that should be included, empty to include all storages. If the array is
   * empty, it has the same effect as if the annotation would just not be specified.
   * <p>
   * For example "local" as the value would specify that the value would be only stored in the
   * default FlintMC storage.
   *
