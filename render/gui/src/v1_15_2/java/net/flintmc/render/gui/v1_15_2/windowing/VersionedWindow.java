...
public class VersionedWindow implements InternalWindow {
  ...
  /**
   * Creates a new OpenGL window using GLFW. Constructor for assisted factory at {@link Window.Factory#create(String,
   * int, int)}.
   *
  ...
  public float[] getSize() {
    ...
    try(MemoryStack stack = MemoryStack.stackPush()) {
      // Reserve space for 2 ints on the stack, will be deallocated automatically when the try-with-resources exits
      IntBuffer buffer = stack.mallocInt(2);

      // Write the width into index 0 and the height into position 1 of the buffer
      glfwGetWindowSize(ensureHandle(), (IntBuffer) buffer.slice().position(0), (IntBuffer) buffer.slice().position(1));
   ...
   *
   * @param destroyWindow If {@code true}, the GLFW window is destroyed, if {@code false}, only the {@link #handle}
   *                      field is zeroed out
   */
